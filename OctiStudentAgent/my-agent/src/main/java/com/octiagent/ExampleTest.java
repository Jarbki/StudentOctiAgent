 ...
 * 
 * This test will evaluate the performance of your custom-built StudentAgent in
 * the Octi game.
 * The purpose is to determine if the agent can achieve a specified win rate or
 * better against
...
public class ExampleTest {
	public static void main(String[] args) {
		...
		/**
		 * This line initializes a performance test for the `myAgent` instance and
		 * immediately runs it with default settings.
		 * 
		 * Breakdown:
		 * 1. `PerformanceTest.test(myAgent)`: This part of the code creates a new
		 * performance test for the `myAgent` instance.
		 * 
		 * 2. `.run()`: This method executes the performance test that was set up in the
		 * previous step. By not specifying
		 * any other configuration methods (like `.random()`, `.color()`, etc.), you are
		...
		// 7. The performance test will evaluate if the agent can win at least 50% (0.5)
		// of its games.
		PerformanceTest.test(myAgent).random(3).color("black")
				.matches(20).timeout(500).level(2).run(0.85);
